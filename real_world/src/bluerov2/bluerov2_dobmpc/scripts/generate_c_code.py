...
def main():
    ...
    # set cost
    W_x = np.diag([1000, 1000, 1000, 10, 10, 150, 50, 50, 50, 5, 5, 30])
    #W_x = np.diag([1000, 1000, 1000, 20, 20, 150, 50, 50, 50, 10, 10, 200])    #Q_mat
    
    # W_x = np.diag([9500, 9500, 3000, 15, 15, 300, 100, 100, 50, 8, 8, 60])    #Q_mat
          
   
    # 增加控制权重以获得更积极的控制响应
    W_u = np.diag([0.001, 0.001, 0.001, 0.01])  # 降低控制惩罚，允许更大控制输出
    # W_u = np.diag([0.011, 0.011, 0.008, 0.12])  # 原始设置 - 控制过于保守
    # W_u = np.diag([0.009, 0.009, 0.008, 0.12])    #R_mat not stable
    # W_u = np.diag([0.007, 0.007, 0.008, 0.12])  not good    
    # W_u = np.diag([0.005, 0.005, 0.008, 0.12])  not good
      #R_mat
        #R_mat
    # W_u = np.diag([0.00913, 0.00913, 0.00457, 0.15656])        #R_mat
    ...
    # set constraints
    u_min = np.array([-104.65, -104.65, -148, -25.27])
    u_max = np.array([104.65, 104.65, 148, 25.27])
