...
class BLUEROV2_DOB{
    ...
    // uuv_gazebo_ros_plugins_msgs::FloatStamped thrust0;  // 注释掉推进器消息
    ...
    // nav_msgs::Odometry velocity_msg;
    // nav_msgs::Odometry body_velocity_msg;
    geometry_msgs::Twist cmd_vel_msg;  // 真实机器人控制用
    std::vector<ros::Subscriber> subscribers;
    // uuv_gazebo_ros_plugins_msgs::FloatStamped control_input0;  // 注释掉不必要的调试消息
    ...
    Matrix<double,6,8> K;           // propulsion matrix
    ...
    // Numerical velocity calculation variables
    ...
    Matrix<double,8,1> meas_u;      // inputs
    ...
    // ros::Publisher thrust0_pub;  // 注释掉推进器话题，真实机器人使用cmd_vel
    ...
    // ros::Publisher control_input0_pub;  // 注释掉不必要的调试话题
    ...
    // ros::Publisher body_velocity_pub;
    ros::Publisher cmd_vel_pub;  // 真实机器人控制用
    ...
    bool use_dynamic_trajectory = true;  // 是否使用动态轨迹生成
    bool trajectory_initialized = false; // 轨迹是否已初始化
    double start_x, start_y, start_z;    // 起始位置
    double sample_time = 0.05;           // 采样时间
    double rectangle_speed = 0.15;       // 矩形轨迹速度 m/s
    double circle_radius = 0.2;         // 圆形轨迹半径 m
    double circle_speed = 0.07;          // 圆形轨迹速度 m/s
    bool use_circle_trajectory = true;   // 是否使用圆形轨迹（true=圆形，false=矩形）
    ...
    void generateDynamicRectangleTrajectory();              // generate dynamic rectangle trajectory
    void generateDynamicCircleTrajectory();                 // generate dynamic circle trajectory
